...
- `patch_auth_upgrade.sql` drops every allow-all anon policy and adds real RLS:
  read = any signed-in staff; instructions/approve/assign/settings/centres/compliance
  items = admin only (`is_admin()`); data + files = admin or that actionable's assigned
  team (`is_on_actionable()`); updates insert as yourself only. Anon key alone: zero rows.
- On this build the compliance auto-spawn engine runs only in the admin's browser
  (spawning inserts actionables, which RLS restricts to admin).
